...
#[cfg(test)]
mod tests {
    // Tests remain unchanged; they are already comprehensive.
    use super::*;
    use std::fs::File;
    use std::io::Write;
    ...
    #[test]
    fn test_create_parent_dir_existing() {
        let tmp = NamedTempFile::new().unwrap();
        let path = tmp.path().to_path_buf();
    ...
    #[test]
    fn test_create_parent_dir_new() {
        let dir = tempdir().unwrap();
        let deep_path = dir.path().join("a/b/c/file.txt");
        create_parent_dir(&deep_path).unwrap();
        assert!(deep_path.parent().unwrap().exists());
    }
    ...
    #[test]
    fn test_collect_files_directory_without_recursive() {
        let dir = tempdir().unwrap();
        let paths = vec![dir.path().to_path_buf()];
        let err = collect_files(&paths, false).unwrap_err();
        assert!(matches!(err, AppError::DirectoryNotAllowed(_)));
    }
    ...
    fn test_collect_files_mixed() {
        ...
        let paths = vec![tmp.path().to_path_buf(), dir.path().to_path_buf()];
        let files = collect_files(&paths, true).unwrap();
        assert_eq!(files.len(), 3);
        assert!(files.contains(&tmp.path().to_path_buf()));
        assert!(files.contains(&file1));
        assert!(files.contains(&file2));
    }
